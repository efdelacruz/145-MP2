...
public class MyClient extends JFrame implements KeyListener{
    ...
    public void init_window(MyClient c){
        ...
        bet_on = new JTextArea();
        bet_amt = new JTextArea();
        ip = new JTextArea();
        port = new JTextArea();
        bet_on.setSize(50, 25);
        bet_on.setEditable(true);
        bet_on.setLocation(375, 65);
        bet_amt.setSize(150, 25);
        bet_amt.setEditable(true);
        bet_amt.setLocation(325, 105);
        //first textarea: ip
        ip.setSize(200,25);
        ip.setEditable(true);
        ip.setLocation(300, 145);
        //second textarea: port
        port.setSize(200,25);
        port.setEditable(true);
        port.setLocation(300,185);
        //set frogs
        ...
        //show frame
        c.add(bet_on); c.add(bet_amt); c.add(win);
        c.add(ip);
        c.add(port);
        c.add(frog1); c.add(A);
        c.add(frog2); c.add(B);
    ...
    public void keyPressed(KeyEvent e){ 
        if(e.getKeyCode()==KeyEvent.VK_ENTER){
            try{
                ...
                connect.sendMessage(bet_amt.getText());
                FrogMover fm = new FrogMover(frog1,frog2,frog3,frog4, xa, xb, xc, xd, ya, yb, yc, yd, bet_on.getText(), bet_amt.getText(), win);
                fm.start();
                ClientListener cl = new ClientListener(fm, connect);
                cl.start();
                bet_on.setEditable(false);
                bet_amt.setEditable(false);
                this.remove(ip);
                this.remove(port);
